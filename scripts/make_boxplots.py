...
Usage:
    python make_boxplots.py --variable pwat
...
def collect_data(variable):
    ...
    for ari in ARI_DURS:
        ...
        if is_wspd:
            level = variable.replace("wspd", "")      # e.g. "850" or "925"
            u_name = f"u{level}"
            v_name = f"v{level}"
        elif is_mt:
            level = variable.replace("mt", "")            # e.g. "850" or "925"
            q_name = f"q{level}"                          # specific humidity
            u_name = f"u{level}"
            v_name = f"v{level}"
        ...
        for month in range(1, 13):
            ...
            if is_wspd or is_mt:
                # paths for u and v
                u_fp = os.path.join(INPUT_BASE, u_name, f"{ari}yrARI",
                                    f"hrrr_3h{ari}yrARI_{u_name}_2024{month:02d}.nc")
                v_fp = os.path.join(INPUT_BASE, v_name, f"{ari}yrARI",
                                    f"hrrr_3h{ari}yrARI_{v_name}_2024{month:02d}.nc")
                ds = xr.open_dataset(u_fp)
                ds_v = xr.open_dataset(v_fp)
                
                if is_mt:
                    q_fp = os.path.join(INPUT_BASE, q_name,  f"{ari}yrARI",
                                        f"hrrr_3h{ari}yrARI_{q_name}_2024{month:02d}.nc")
                    ds_q = xr.open_dataset(q_fp)
                
            else:
                filepath = os.path.join(
                    subdir,
                    f'hrrr_3h{ari}yrARI_{variable}_2024{month:02d}.nc'
                )
                if not os.path.isfile(filepath):
                    print(f"Warning: file not found {filepath}")
                    continue
                ds = xr.open_dataset(filepath)
            ysz, xsz = ds.dims['y'], ds.dims['x']
            i, j = ysz // 2, xsz // 2
            times = ds['time'].values
            lats = ds['centroid_lat'].values
            lons = ds['centroid_lon'].values

            for evt in range(ds.dims['event']):
                lat, lon = float(lats[evt]), float(lons[evt])
                ...
                for hr in HOURS:
                    ...
                    if is_wspd:
                        uval = ds[u_name].values[evt, t_index, i, j]
                        vval = ds_v[v_name].values[evt, t_index, i, j]
                        val = np.sqrt(uval**2 + vval**2) * 2.23694
                    elif is_mt:
                        q_slice = ds_q[q_name].isel(event=evt, time=t_index, y=i, x=j)
                        u_slice = ds[u_name].isel(event=evt, time=t_index, y=i, x=j)
                        v_slice = ds_v[v_name].isel(event=evt, time=t_index, y=i, x=j)
                        # call imported function
                        mf = instantaneous_moisture_flux(q_slice, u_slice, v_slice)
                        val = mf.values
                    else:
                        val = ds[variable].values[evt, t_index, i, j]
                    if np.isnan(val):
                        continue
                    # convert pwat to inches
                    if variable == 'pwat':
                        val *= MM_TO_INCH
                    records.append({
                        'ari_dur': ari,
...
def plot_by_region(df, variable):
    ...
    for season in seasons:
        ...
        fig.suptitle(f"{pretty_var(variable)} by Region — Season {season}", fontsize=16)
        fig.savefig(os.path.join(OUTPUT_DIR, f'{variable}_season_{season}_byRegion.pdf'))
        plt.close(fig)
...
def plot_by_season(df, variable):
    ...
    for region in regions:
        ...
        fig.suptitle(f"{pretty_var(variable)} by Season — Region {region}", fontsize=16)
        fig.savefig(os.path.join(OUTPUT_DIR, f'{variable}_region_{region}_bySeason.pdf'))
        plt.close(fig)
...
def plot_by_hour(df, variable):
    ...
    for season in seasons:
        ...
        fig.suptitle(f"{pretty_var(variable)} by Hour offset — Season {season}", fontsize=16)
        fig.savefig(os.path.join(OUTPUT_DIR, f'{variable}_season_{season}_byHour.pdf'))
        plt.close(fig)
